...
================================================================================
DESCRIPTION:
    Loads MobileNetV3-based classifier to predict damage types and chair parts.
    Produces Stage I JSON with detected_damages, detected_parts, and simple
    detected_pairs for downstream planning.
...
OUTPUTS:
    {
      ...
      "detected_pairs":   [{"part": "...", "damage_type": "...", "damage_confidence": float, "part_confidence": float}]
    }
...
def detect_damage_and_parts(image_path: str, model_path: str, thresh: float = 0.5):
    ...
    detected_damages = [
        ...
        for i in range(len(DAMAGE_CLASSES)) if damage_probs[i] >= thresh
    ]
    detected_parts = [
        {"part": PART_CLASSES[i], "confidence": float(part_probs[i])}
        for i in range(len(PART_CLASSES)) if part_probs[i] >= thresh
    ]

    # Simple pair construction: assign highest-confidence damage to each detected part
    pairs = []
